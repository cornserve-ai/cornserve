...
async def run(
    ...
    if not overwrite:
        ...
        if not configs:
            ...
            eric_tput = eric_exp.load()["metrics"]["request_throughput"]
            el_l_tput = el_l_exp.load()["metrics"]["request_throughput"]

            # we only consider 8 gpu case
            print("4 l_{epd} vs 2e+3l_{pd}")
            print(4*vllm_tput, "vs", min(2*eric_tput,3*el_l_tput))
            print("4 l_{epd} vs 4e+2l_{pd}")
            print(4*vllm_tput, "vs", min(4*eric_tput,2*el_l_tput))
            exit(0)
    ...
    for cfg in configs:
        ...
        tput = output_data["metrics"]["request_throughput"]
        print(f"Throughput: {tput:.2f} requests/s")
