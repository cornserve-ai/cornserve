...
async def run(
    ...
    audio_config = EricConfig(num_replicas=1, modality="audio")
    ...
    audio_workload_config = OmniConfig(
        include_image=False,
        include_audio=True,
        include_video=False,
        image_width=image_width,
    ...
    audio_sampled_requests = smaple_requests(num_requests, audio_workload_config)
    ...
    audio_exp_config = ExperimentConfig(
        backend_config=audio_config,
        app_id=audio_eric,
        model_id=model_id,
        gpu_type=gpu_type,
        dataset="omni",
        workload_config=audio_workload_config,
        request_rate=audio_request_rate,
        use_synthesized_data=False,
    )
    configs.append((audio_exp_config, audio_sampled_requests))

    if el and llm_request_rate > 0:
