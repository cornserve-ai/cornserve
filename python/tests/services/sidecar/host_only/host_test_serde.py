...
class DockerTestManager:
    ...
    def start_server_container(self) -> subprocess.Popen:
        ...
        base_cmd = [
            ...
            CONTAINER_IMAGE,
            "python", "-u", "server.py", "--clients", str(NUM_GPUS)  # -u for unbuffered output
        ]
...
def test_multi_container_serde(docker_args: Optional[List[str]] = None, 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--docker-args", nargs="*", 
                       help="Additional Docker arguments")
    parser.add_argument("--mounts", nargs="*", 
                       help="Additional volume mounts (format: host:container:options)")
    parser.add_argument("--gpus", type=int, default=4,
                       help="Number of GPUs to test with")
    ...
    test_multi_container_serde(
        docker_args=custom_docker_args,
        additional_mounts=custom_mounts
    )
