...
def test_weight_loading() -> None:
    ...
    hf_model = Qwen3VLForConditionalGeneration.from_pretrained(model_id, torch_dtype="auto").visual
...
def test_hf_reference(test_images: list[ModalityData], test_videos: list[ModalityData], dump_tensors: str) -> None:
    ...
    def transform_hf_output(output, grid_thw):
        ...
        for i, feat in enumerate(deepstack_features_list):
            print(f"Deepstack feature {i} shape: {feat.shape}")
