...
from cornserve.task.builtins.llm import (
    ...
    NcclDecodeLLMUnitTask,
    NcclPrefillLLMUnitTask,
...
class NcclPrefillVLLMDescriptor(
    TaskExecutionDescriptor[
        NcclPrefillLLMUnitTask,
        OpenAIChatCompletionRequest,
        PrefillChatCompletionResponse,
    ...
    NCCL_ZMQ_BASE_PORT: ClassVar[int] = 5665

    def create_executor_name(self) -> str:
        """Create a name for the task executor."""
        return _nccl_prefill_executor_name(self.task.model_id)
    ...
    def get_service_ports(self, gpus: list[GPU]) -> list[tuple[str, int]]:
        """Get the additional service ports for the task executor."""
        return [
            ("nccl-zmq", self.NCCL_ZMQ_BASE_PORT + gpus[0].global_rank),
        ]
    ...
    def get_container_args(self, gpus: list[GPU], port: int) -> list[str]:
        ...
        args = [
            ...
            "--trust-remote-code",
            "--kv-transfer-config",
            f'{{"kv_connector":"P2pNcclConnector","kv_role":"kv_producer","kv_buffer_size":"1e1","kv_port":"{self.NCCL_ZMQ_BASE_PORT}","kv_connector_extra_config":{{"http_port":"8000","send_type":"PUT_ASYNC","nccl_num_channels":"16"}}}}',
            # need to forward KV transfer parameters to a decode instance
            "--cornserve-sidecar-ranks",
            *[str(gpu.global_rank) for gpu in gpus],
            # here we synchronize arguments with the base descriptor
    ...
    def to_request(
        ...
        if (kv_cache := task_output.kv_cache) is not None:
            assert kv_cache.src_sidecar_ranks is not None
            assert kv_cache.dst_sidecar_ranks is not None
            assert len(kv_cache.dst_sidecar_ranks) == 1
            # we construct the new request_id
            prefill_te_name = _nccl_prefill_executor_name(self.task.model_id)
            prefill_service_name = _get_service_name(prefill_te_name, sidecar_ranks=kv_cache.src_sidecar_ranks)
            decode_te_name = _nccl_decode_executor_name(self.task.model_id)
            decode_service_name = _get_service_name(decode_te_name, sidecar_ranks=kv_cache.dst_sidecar_ranks[0])
            prefill_zmq_addr = f"{prefill_service_name}:{self.NCCL_ZMQ_BASE_PORT+ min(kv_cache.src_sidecar_ranks)}"
            decode_zmq_addr = f"{decode_service_name}:{self.NCCL_ZMQ_BASE_PORT+ min(kv_cache.dst_sidecar_ranks[0])}"
            request_id = (
                f"___prefill_addr_{prefill_zmq_addr}___decode_addr_"
                f"{decode_zmq_addr}_{kv_cache.id}___"
            )
...
DESCRIPTOR_REGISTRY.register(NcclPrefillLLMUnitTask, NcclPrefillVLLMDescriptor, default=True)

def _nccl_prefill_executor_name(model_id: str) -> str:
    return "-".join(["nccl-prefill", model_id.split("/")[-1]]).lower()

def _nccl_decode_executor_name(model_id: str) -> str:
    return "-".join(["nccl-decode", model_id.split("/")[-1]]).lower()

def _get_service_name(executor_name: str, sidecar_ranks: list[int]) -> str:
    executor_id = "-".join([executor_name, *(str(rank) for rank in sidecar_ranks)])
    return to_strict_k8s_name(f"te-{executor_id}")

class NcclDecodeVLLMDescriptor(
    TaskExecutionDescriptor[
        NcclDecodeLLMUnitTask,
        OpenAIChatCompletionRequest,
        Stream[OpenAIChatCompletionChunk],
    ...
    def create_executor_name(self) -> str:
        """Create a name for the task executor."""
        return _nccl_decode_executor_name(self.task.model_id)
    ...
    def get_container_args(self, gpus: list[GPU], port: int) -> list[str]:
        ...
        args = [
            ...
            "--trust-remote-code",
            "--kv-transfer-config",
            f'{{"kv_connector":"P2pNcclConnector","kv_role":"kv_consumer","kv_buffer_size":"8e9","kv_port":"{self.NCCL_ZMQ_BASE_PORT}","kv_connector_extra_config":{{"http_port":"8000","send_type":"PUT_ASYNC","nccl_num_channels":"16"}}}}',
            # need to receive KV transfer parameters from a decode instance
            "--cornserve-sidecar-ranks",
            *[str(gpu.global_rank) for gpu in gpus],
            # here we synchronize arguments with the base descriptor
    ...
    def to_request(
        ...
        request = task_input.model_dump(exclude={"cornserve_embeddings", "cornserve_kv_transfer_params"})
        if (kv_cache := task_input.cornserve_kv_cache) is not None:
            assert kv_cache.src_sidecar_ranks is not None
            assert kv_cache.dst_sidecar_ranks is not None
            assert len(kv_cache.dst_sidecar_ranks) == 1
            # we construct the new request_id
...
DESCRIPTOR_REGISTRY.register(NcclDecodeLLMUnitTask, NcclDecodeVLLMDescriptor, default=True)
