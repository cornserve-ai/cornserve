...
class PrefillChatCompletionResponse(TaskOutput):
    ...
    # for nccl-based pd
    kv_cache: DataForward[Tensor] | None = None
...
class NcclPrefillLLMUnitTask(UnitTask[OpenAIChatCompletionRequest, PrefillChatCompletionResponse]):
    """A task that invokes a vLLM to perform prefill."""
    ...
    def make_name(self) -> str:
        """Create a concise string representation of the task."""
        return f"nccl-prefill-{self.model_id.split('/')[-1].lower()}"

    def make_record_output(self, task_input: OpenAIChatCompletionRequest) -> PrefillChatCompletionResponse:
        """Create a mock task output object for invocation recording."""
        if self.send_hidden_states:
            return PrefillChatCompletionResponse(
                hidden_states=DataForward[Tensor](),
                kv_cache=DataForward[Tensor](),
            )
        return PrefillChatCompletionResponse(kv_cache=DataForward[Tensor]())
...
class NcclDecodeLLMUnitTask(UnitTask[OpenAIChatCompletionRequest, Stream[OpenAIChatCompletionChunk]]):
    """A task that invokes a vLLM decoder and returns a stream of chat completion chunks."""
    ...
    def make_name(self) -> str:
        """Create a concise string representation of the task."""
        return f"nccl-decode-{self.model_id.split('/')[-1].lower()}"
...
class NcclDisaggregatedMLLMTask(Task[OpenAIChatCompletionRequest, Stream[OpenAIChatCompletionChunk]]):
    """A task that invokes a Multimodal LLM, with disaggregated prefill and decode in LLM.
    ...
    def post_init(self) -> None:
        ...
        self.prefill = NcclPrefillLLMUnitTask(model_id=self.model_id, receive_embeddings=self.encoder_fission)
        self.decode = NcclDecodeLLMUnitTask(model_id=self.model_id, receive_embeddings=self.encoder_fission)
    ...
    def invoke(self, task_input: OpenAIChatCompletionRequest) -> Stream[OpenAIChatCompletionChunk]:
        ...
        # due to a potential bug in vLLM
        decode_input = task_input.model_copy(deep=True)
        decode_input.cornserve_kv_cache = prefill_output.kv_cache
