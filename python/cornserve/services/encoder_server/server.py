...
async def main() -> None:
    ...
    while True:
        ...
        #     if SHARD_RANK == 0:
        #         # dummy_tensor = torch.ones(tensor_shape, device=device, dtype=dtype)
        #         dummy_tensor = torch.arange(1, num_elements + 1, device=device, dtype=dtype).reshape(tensor_shape)
        #     else:
        #         dummy_tensor = torch.arange(1+num_elements, 2*num_elements + 1, device=device, dtype=dtype).reshape(tensor_shape)
