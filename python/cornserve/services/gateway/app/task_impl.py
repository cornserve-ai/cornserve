...
async def llm_task_invoke(
    ...
    """
    with tracer.start_as_current_span("llm_task_invoke") as span:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            span.add_event("Send to task dispatcher", attributes={"task_id": self.id})
            response = await client.post(
                url=f"http://{constants.K8S_TASK_DISPATCHER_HTTP_URL}/task",
                json=request.model_dump(),
            )
            response.raise_for_status()
            span.add_event("Received response", attributes={"task_id": self.id})
